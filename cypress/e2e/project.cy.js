...
describe('2 - Crie um parágrafo com `id` `carta-gerada` onde a pessoa usuária verá o resultado de sua carta misteriosa', () => {
  beforeEach(() => {
    cy.visit('./index.html');
  });
...
describe('3 - Crie um botão com `ID` `criar-carta` que deve gerar a carta misteriosa ao ser clicado', () => {
  beforeEach(() => {
    cy.visit('./index.html');
  });
...
describe('4 - Faça com que o input com `id` `carta-texto` permaneça com o texto digitado após clicar no botão de `id` `criar-carta`', () => {
  beforeEach(() => {
    cy.visit('./index.html');
  });
...
describe("5 - Crie a mensagem 'Por favor, digite o conteúdo da carta!' para caso a pessoa usuária não preencha o campo ou preencha apenas com espaços em branco", () => {
  beforeEach(() => {
    cy.visit('./index.html');
  });
...
describe('12 - Crie a classe `rotateleft`', () => {
  ...
  it('Será validado se a classe `rotateleft` possui a propriedade `transform` igual a "rotate(-5deg)"', () => {
    const properties =[{
      key: 'transform',
      // the value of `rotate(-5deg)` is `matrix(0.996195, -0.0871557, 0.0871557, 0.996195, 0, 0)`
...
describe('13 - Crie a classe `rotateright`', () => {
  ...
  it('Será validado se a classe `rotateright` possui a propriedade `transform` igual a "rotate(5deg)', () => {
    const properties =[{
      key: 'transform',
      // the value of `rotate(5deg)` is `matrix(0.996195, 0.0871557, -0.0871557, 0.996195, 0, 0)`
      value: 'matrix(0.996195, 0.0871557, -0.0871557, 0.996195, 0, 0)'
...
describe('14 - Crie a classe `skewleft`', () => {
  ...
  it('Será validado se a classe `skewleft` possui a propriedade `transform` igual a "skewX(10deg)"', () => {
    const properties =[{
      key: 'transform',
      value: 'matrix(1, 0, 0.176327, 1, 0, 0)' // the value of `skewX(10deg)` is `matrix(1, 0, 0.176327, 1, 0, 0)`
    }];
...
describe('15 - Crie a classe `skewright`', () => {
  ...
  it('Será validado se a classe `skewright` possui a propriedade `transform` igual a "skewX(-10deg)"', () => {
    const properties =[{
      key: 'transform',
      value: 'matrix(1, 0, -0.176327, 1, 0, 0)' // the value of `skewX(-10deg)` is `matrix(1, 0, -0.176327, 1, 0, 0)`
    }];
...
describe('17 - Adicione a possibilidade de alterar o estilo de uma palavra específica ao clicar nela', () => {
  ...
  it('Será validado se ao clicar em uma carta uma nova lista aleatória de classes é gerada', () => {
    cy.get(wordSelector).then(($el) => {
      const span = $el[0];
      const klasses = span.classList;
      first.push(...klasses);
      expect(first.length).to.be.equal(4);
      cy.get(wordSelector).click().then(($elem) => {
        const span = $elem[0];
        const secondKlasses = span.classList;
        second.push(...secondKlasses);
        expect(first).to.not.deep.equal(second);
      });
    });
  });
  ...
  it('Será validado se ao clicar novamente na mesma carta uma nova lista aleatória de classes é gerada e se a nova lista é diferente da lista anterior', () => {
      cy.get(wordSelector).click().then(($elem) => {
        const span = $elem[0];
        const thirdKlasses = span.classList;
        third.push(...thirdKlasses);
        expect(second).to.not.deep.equal(third);
        expect(second.length).to.be.equal(4);
        expect(third.length).to.be.equal(4);
      });
    });
  });

describe('18 - Crie um parágrafo com o `id` `carta-contador` que deve contar o número de palavras geradas naquela carta', () => {
  beforeEach(() => {
    cy.visit('./index.html');
  });
